...
# LOB level
lob_level <- gs_title("MNL COE Outflows")
...
site_data_diff<-sqldf("Select 
                      week as Week,
                      'COE-UBR-MNL' as Site,
                      (bliss_solves+jira_solves) as Actual_Solved,
                      (csat_score) as 'Actual_Responses',
                      ...
                      0 as 'Actual_R2',
                      0 as 'R2_Delta',
                      round(Target_TPH,2) as Target_TPH,
                      round(TPH,2) as Actual_TPH,
                      round((TPH-Target_TPH),2) as 'TPH_Delta',
                      round(Target_res_eff,2) as Target_Resolution_Efficiency,
                      round(res_eff,2) as Actual_Resolution_Efficiency,
                      round((res_eff-Target_res_eff),2) as 'Resolution_Efficiency_Delta',
                      round(bliss_handle_time,2) as 'Bliss_Handle_Time',
                      0 as 'Target_Liveops_AHT',
                      0 as 'Actual_Liveops_AHT',
                      0 as 'Liveops_AHT_Delta'
